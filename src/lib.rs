...
async fn tunnel(req: Request, mut cx: RouteContext<Config>) -> Result<Response> {
    ...
    match proxyip_lower.as_str() {
        "vmess" => {
            ...
            proxyip = "AT".to_string(); // map vmess -> use KV key "ID"
        }
        "vless" => {
            protocol = "vless".to_string();
            proxyip = "AT".to_string(); // map vless -> use KV key "ID"
        }
        "trojan" => {
            protocol = "trojan".to_string();
            proxyip = "AT".to_string(); // map trojan -> use KV key "ID"
        }
        "shadowsocks" => {
            protocol = "shadowsocks".to_string();
            proxyip = "AT".to_string();
        }
        _ => {
